...
每个行动对范围产生乘法压缩，系数来自GTO求解器研究中的典型下注频率：
  - 下注/加注 ~0.45 — 对手以约45-55%的范围下注（价值+诈唬极化）
  - 跟注     ~0.75 — 跟注范围更宽（包含跟注摸牌和中强度手牌）
  - 过牌     ~0.92 — 过牌范围信息量较少（含慢打和弱手）

胜率折扣
--------
若对手持有全部手牌中最强X%的组合，我们对随机手的MC胜率会系统性高估。
折扣曲线通过以下校准点进行分段线性插值（参考PioSolver研究数据）：
...
示例（65o手牌分析）：
  对手在翻牌/转牌/河牌连续下注 → range_fraction ≈ 0.08
  equity_discount ≈ 0.57
  raw_equity = 0.41 → effective_equity = 0.41 * 0.57 = 0.23
  底池赔率 = 0.22 → 0.23 > 0.22，仍然跟注
  但面对河牌加注，range_fraction 进一步压缩 → 0.08 * 0.45 = 0.036
  equity_discount ≈ 0.45 → effective_equity = 0.41 * 0.45 = 0.18 < 0.22 → 弃牌 ✓
"""
