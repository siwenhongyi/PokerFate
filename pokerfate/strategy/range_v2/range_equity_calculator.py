...
class RangeEquityCalculator:
    ...
    def weighted_equity(
        ...
        if num_opponents <= 1:
            ...
            sampled = np.random.choice(1326, size=n_samples, replace=True, p=valid)
            ...
            for idx in sampled:
                ...
                if is_river:
                    my_score = HandEvaluator.evaluate(list(my_hand) + list(board))
                    opp_score = HandEvaluator.evaluate(opp_cards + list(board))
                else:
                    opp_ints = {c1, c2}
                    avail = [c for c in deck if c not in opp_ints]
                    if len(avail) < board_needed:
                        continue
                    runout = random.sample(avail, board_needed)
                    full_board = list(board) + [hcm.int_to_card(r) for r in runout]
                    my_score = HandEvaluator.evaluate(list(my_hand) + full_board)
                    opp_score = HandEvaluator.evaluate(opp_cards + full_board)
            ...
            for _ in range(n_samples):
                ...
                for _o in range(num_opponents):
                    ...
                    if s < 1e-15:
                        ...
                        break
                    w_copy /= s
                    idx = np.random.choice(1326, p=w_copy)
                    c1, c2 = hcm.ALL_COMBOS[idx]
                ...
                valid_count += 1
                my_score = HandEvaluator.evaluate(list(my_hand) + full_board)
                opp_scores = [HandEvaluator.evaluate(h + full_board) for h in opp_hands]
