local P = class("BunnyGirlExchange", require("app.views.Shop.ShopExchangeBatch"))
...
function P:onShow()
    ...
    if self._params.isSingle then
 	...
 	local grayPropIcon = self:find("ExchangeGrayButton/Pri/bunnygirl_tc_icon_01", self.Batch)
	local propIcon = self:find("ExchangeButton/Pri/bunnygirl_tc_icon_01", self.Batch)
	local singleIcon = self:find("TextPrice/bunnygirl_tc_icon_01", self.Single)
	local icon = ThemeModel:getThemeIcon()
	bee.setIcon(grayPropIcon, icon)
	bee.setIcon(propIcon, icon)
	bee.setIcon(singleIcon, icon)
end
...
function P:refreshText()
	local priTran = self:find("Pri", self.ExchangeButton):GetComponent("RectTransform")
	CU.UI.LayoutRebuilder.ForceRebuildLayoutImmediate(priTran)
end
