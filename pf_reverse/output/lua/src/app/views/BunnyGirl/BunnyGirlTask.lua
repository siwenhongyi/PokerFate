local P = class("BunnyGirlTask", require("app.views.BunnyGirl.BunnyGirlBase"))

function P:onAwake()
    P.super.onAwake(self)
    self._tips = {"LAB_STORY_DIALOGUE_S4_1002_01","LAB_STORY_DIALOGUE_S4_1002_02","LAB_STORY_DIALOGUE_S4_1002_03"}
    ...
    bee.addClick(self.ConfirmButton, function()
        if bee.checkCd("BunnyGirlTask_ConfirmButton", 1) then
...
function P:onShow()
    P.super.onShow(self)
    bee.invoke(self.CharacterImage, "setSkin", tpl_character_skin[100205], true)
...
function P:refreshItem(data, item, isInit, index)
    ...
    if self._isInInit and isInit then
        self:once(0.1 * (index - 1), function()
            ...
            self:playAnimator("UI_1_SchoolTask_Item", Ani_root)
        end)
        Ani_root:SetActive(false)
    else
        Ani_root:SetActive(true)
        self:playAnimator("UI_1_SchoolTask_Item_idle", Ani_root)
    end

    local cur, sum = task.current_value, task.value[3] or task.value[2] or task.value[1]
    cur = math.min(cur, sum)
    local des = TaskModel:getTaskDesc(d)
    local tarStr = des .. string.format(" (%s/%s)", (cur < sum and string.format("<color=#FF0000>%s</color>", cur) or cur), sum)
    bee.setText(self:find("Text", Ani_root), tarStr)
    bee.setIcon(self:find("Item/bunnygirl_icon_champagne", Ani_root), tpl_props[d.rewards[1]].icon)
    bee.setText(self:find("Item/Value", Ani_root), "x" .. d.rewards[2])
    ...
    self:find("bunnygirl_task_done", Ani_root):SetActive(task.status == TaskStatus.Received)
    self:find("bunnygirl_task_frame_mask", Ani_root):SetActive(task.status == TaskStatus.Received)
    ...
    bee.addClick(self:find("Item", Ani_root), function()
        Game:playSound("ui_button_confirm")
        UiManager:showUI("CommonItemTip", {data = ItemModel:getItem(ThemeModel:getItemId(), true), target = self:find("Item/bunnygirl_icon_champagne", Ani_root)})
    end, true)
...
function P:evt_PinballActionRSP()
    self:checkReqTask(101, GAME_GAME_TYPE.SIDE_GAME_PINBALL_GAME)
end

function P:evt_PinballWin()
    self:checkReqTask(104, GAME_GAME_TYPE.SIDE_GAME_PINBALL_GAME)
end
...
function P:checkReqTask(taskType, gameType)
    ...
    for _,v in pairs(tasks) do
        ...
        if cfg.task_type == taskType and cfg.value[1] == gameType then
            reqable = true
            break
        end
    end
