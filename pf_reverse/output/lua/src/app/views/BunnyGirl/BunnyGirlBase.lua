local P = class("BunnyGirlBase", UiFullView)
...
function P:onAwake()
    ...
    self.PropButton = self:find("Currency/bunnygirl_icon_champagne", self.RightTop)
    self.TextCount = self:find("Currency/Value", self.RightTop)
    self.PlusFrameButton = self:find("Currency/bunnygirl_currency_bg", self.RightTop)
    self.PlusButton = self:find("Currency/bunnygirl_currency_plussign", self.RightTop)
    ...
    bee.addClick(self.TipsButton, function()
        Game:playSound("ui_button_confirm")
        UiManager:showUI("BunnyGirlRules")
    end)
    ...
    bee.addClick(self.RoleButton, function()
        if bee.checkCd("BunnyGirl_RoleButton", 1) then
            self:showRoleTip()
        end
    end)

    bee.setIcon(self.PropButton, ThemeModel:getThemeIcon())
