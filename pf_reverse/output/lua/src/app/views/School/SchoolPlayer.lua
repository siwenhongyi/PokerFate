local P = class("SchoolPlayer", UiDialog)
...
function P:onAwake()
    ...
    bee.addClick(self.btn_choose, function()
        Game:playSound("ui_button_confirm")
        if bee.checkCd("SchoolPlayer_btn_choose", 2) then
...
function P:refreshItem(data, item)
    ...
    local UsingFrame = self:find("school_player_frame_chose_02", item)
    local NormalFrame = self:find("school_player_frame_chose_03", item)
    local SelectFrame = self:find("school_player_frame_chose_01", item)
    local ImageIcon = self:find("school_player_img_character_01", item)
    ...
    bee.setText(self:find("Point/Value", item), data.add / 10 .. "%")
...
function P:refreshSelected(item, id, force)
    ...
    if self._selectedItem then
        self:find("school_player_frame_chose_01", self._selectedItem):SetActive(false)
    end
    self._selectedItem = item
    self._selectedId = id
    self:find("school_player_frame_chose_01", item):SetActive(true)
