local P = class("SchoolShop", require("app.views.School.SchoolBase"))

function P:onAwake()
    P.super.onAwake(self)
    self._tips = {"LAB_STORY_DIALOGUE_S3_1012_01","LAB_STORY_DIALOGUE_S3_1012_02","LAB_STORY_DIALOGUE_S3_1012_03"}

    self.ShopList = self:find("ShopList", self.Right)
    self.Line = self:find("Line1", self.ShopList)
...
function P:onShow()
    P.super.onShow(self)
    bee.invoke(self.CharacterImage, "setSkin", tpl_character_skin[101205], true)
...
function P:initShopItems()
    ...
    for i = 1, lineCount do
        ...
        for _ = 1, itemCount do
            ...
            item.frame = self:find("school_shop_bg_sp_01", itemRoot)
            ...
            item.eyeBtn = self:find("school_shop_btn_view", itemRoot)
            item.mask = self:find("school_shop_img_mask", itemRoot)
...
function P:refreshUI()
    self._datas = ShopModel:getShopExchangeList(self.activityData.shop_type)
...
function P:refreshItem(data, index)
    ...
    bee.setIcon(item.frame, data.cfg.is_special and "School[school_shop_bg_sp_01]" or "School[school_shop_bg_sp_02]")
    ...
    bee.addClick2(item.go, function()
        ...
        UiManager:showUI("SchoolExchange", {data = data, isSingle = data.cfg.batch ~= 1})
    end, true)
...
function P:evt_ItemChangeRSP(msg)
    ...
    for _, v in ipairs(msg.item_list) do
        if table.indexof(items, v.item_id) > 0 then
            bee.showUiTask("SchoolHint", {item = v}, POP_TAG.Reward, LOBBY_POP_PRIORITY.Reward)
