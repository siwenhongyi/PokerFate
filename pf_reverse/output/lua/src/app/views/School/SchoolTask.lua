local P = class("SchoolTask", require("app.views.School.SchoolBase"))

function P:onAwake()
    P.super.onAwake(self)
    self._tips = {"LAB_STORY_DIALOGUE_S3_1006_01","LAB_STORY_DIALOGUE_S3_1006_02","LAB_STORY_DIALOGUE_S3_1006_03"}

    self.TaskList = self:find("TaskList", self.Right)
    self.Item = self:find("Item1", self.TaskList)
    self.Item:SetActive(false)

    -- self._pregressSize = self:find("Ani_root/Task/TaskSlider/school_task_slider_fill", self.Item).transform.sizeDelta
    ...
    bee.addClick(self.ConfirmButton, function()
        if bee.checkCd("SchoolTask_ConfirmButton", 1) then
...
function P:onShow()
    P.super.onShow(self)
    bee.invoke(self.CharacterImage, "setSkin", tpl_character_skin[100605], true)
...
function P:refreshItem(data, item, isInit, index)
    ...
    bee.setText(self:find("Task/TextName", Ani_root), tarStr)
    bee.setIcon(self:find("Item1/school_task_icon_01", Ani_root), tpl_props[d.rewards[1]].icon)
    bee.setText(self:find("Item1/Value", Ani_root), "x" .. d.rewards[2])
    ...
    self:find("school_task_img_done", Ani_root):SetActive(task.status == TaskStatus.Received)
    self:find("school_task_img_mask", Ani_root):SetActive(task.status == TaskStatus.Received)

    -- bee.setText(self:find("Task/TextProgress", Ani_root), "" .. cur .. "/" .. sum)
    -- self:find("Task/TaskSlider/school_task_slider_fill", Ani_root).transform.sizeDelta = bee.v2(self._pregressSize.x * math.min(cur / sum, 1), self._pregressSize.y)
    ...
    bee.addClick(self:find("Item1", Ani_root), function()
        Game:playSound("ui_button_confirm")
        UiManager:showUI("CommonItemTip", {data = ItemModel:getItem(ThemeModel:getItemId(), true), target = self:find("Item1/school_task_icon_01", Ani_root)})
    end, true)
