---@class ButtonInfoRowView
local P = class("ButtonInfoRowView", UiBase)
...
function P:setPos()
	...
	local w, h = self.node.transform.rect.width, self.node.transform.rect.height
	local offset = selfHeight / 2  + targetHeight / 2
	local endPosX, endPosY = self.targetPosition.x, self.targetPosition.y
	...
	--table.Dump({endPosX, endPosY, selfWidth, selfHeight, offset, w, h}, "setpos")
	if endPosY - selfHeight - (offset / 2) < - (h / 2) then
...
function P:setBtns()
	...
	for i = 1, btnCount do
		...
		local btnData = self.btnList[i] or {}
